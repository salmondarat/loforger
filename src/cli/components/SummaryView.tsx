...
export const SummaryView: React.FC<SummaryViewProps> = ({ summary, issues }) => {
  const errors = issues.filter(i => i.severity === 'error');
  const warnings = issues.filter(i => i.severity === 'warning');

  return (
    <Box flexDirection="column">
      <Text bold underline>Project Configuration Summary</Text>
      
      {errors.length > 0 && (
        <Box flexDirection="column" marginY={1}>
          <Text color="red" bold>Errors (must fix before proceeding):</Text>
          {errors.map(issue => (
