...
export const OptionList: React.FC<OptionListProps> = ({ options, type, value, onChange }) => {
      ...
      {options.map((opt, index) => {
        ...
        return (
          <Box key={opt.value}>
            <Text>
              {highlighted ? '>' : ' '} {' '}
              {type === 'multi' ? (selected ? '[x]' : '[ ]') : (selected ? '(*)' : '( )')}
              {' '}
              <Text bold={highlighted}>{opt.label}</Text>
              {opt.badge && <Text color={getBadgeColor(opt.badge)}> [{opt.badge}]</Text>}
            </Text>
