import type { Question, Answers, AnswerValue, Option } from '../types/index.js';
...
export const QUESTIONS: Question[] = [
  ...
  // ══════════════════════════════════════════════════════════════
  {
    ...
    required: true,
    options: [
      { value: 'mvp', label: 'MVP mode', description: 'Start from scratch, fast, minimal config — focus on core features', badge: 'recommended' },
      { value: 'production', label: 'Production mode', description: 'Full setup: CI/CD, monitoring, security hardening, Docker', badge: 'popular' },
      { value: 'extend', label: 'Extend mode', description: 'Project already exists, add new features', badge: 'popular' },
    ],
  },
  ...
  // ══════════════════════════════════════════════════════════════
  {
    ...
    default: 'my-app',
    validate: (val) => {
      if (typeof val !== 'string' || val.trim().length === 0) return 'Project name cannot be empty.';
      if (!/^[a-z0-9-]+$/.test(val as string)) return 'Use lowercase letters, numbers, or hyphens only (kebab-case).';
      return null;
    },
  },
  ...
  // ══════════════════════════════════════════════════════════════
  {
    ...
    required: true,
    options: [
      { value: 'web', label: 'Web app', description: 'Browser-based, responsive, can be SSR or SPA', badge: 'popular' },
      { value: 'api-only', label: 'API / Backend only', description: 'Headless backend, no frontend', badge: 'popular' },
      { value: 'mobile-rn', label: 'Mobile — React Native', description: 'iOS + Android, JavaScript/TypeScript ecosystem', badge: 'popular' },
      { value: 'mobile-flutter', label: 'Mobile — Flutter', description: 'iOS + Android, Dart, single codebase', badge: 'popular' },
    ],
  },
  ...
  // ══════════════════════════════════════════════════════════════
  {
    ...
    default: 'single',
    options: [
      { value: 'single', label: 'Single app', description: 'One folder, one app — simplest', badge: 'recommended' },
      { value: 'turborepo', label: 'Monorepo — Turborepo', description: 'Automatic build cache, simple pipeline', badge: 'recommended' },
    ],
  },
  ...
  // ══════════════════════════════════════════════════════════════
  {
    ...
    options: (answers): Option[] => {
      ...
      if (platform === 'mobile-flutter') {
        return [
          { value: 'dart', label: 'Dart', badge: 'recommended' },
        ];
      }
      
      // Web or API-only
      return [
        { value: 'typescript', label: 'TypeScript', description: 'Default for JS projects', badge: 'recommended' },
        { value: 'javascript', label: 'JavaScript', description: 'No type system' },
        { value: 'python', label: 'Python', description: 'For FastAPI / Django' },
  ...
  // ══════════════════════════════════════════════════════════════
  {
    ...
    options: (answers): Option[] => {
      ...
      if (platform === 'mobile-rn') {
        return [
          { value: 'react-native-expo', label: 'React Native + Expo', description: 'Managed workflow, best DX for RN', badge: 'recommended' },
          { value: 'react-native-bare', label: 'React Native (bare)', description: 'Full control, more native module access' },
        ];
      }
      ...
      if (lang === 'typescript' || lang === 'javascript') {
        return [
          ...
          { value: 'sveltekit', label: 'SvelteKit', description: 'Svelte, lightweight, compiled', badge: 'experimental' },
          { value: 'vite-react', label: 'Vite + React', description: 'Pure SPA, no SSR', badge: 'popular' },
      ...
      if (lang === 'python') {
        return [
          ...
          { value: 'htmx', label: 'HTMX + Jinja2', description: 'Server-rendered with Django/FastAPI' },
        ];
      }
  ...
  // ══════════════════════════════════════════════════════════════
  {
    ...
    options: (answers): Option[] => {
      ...
      if (lang === 'typescript' || lang === 'javascript') {
        return [
          { value: 'nextjs-api', label: 'Next.js API Routes / Server Actions', description: 'Built-in if using Next.js', badge: frontendFW === 'nextjs' ? 'recommended' : 'popular' },
          { value: 'nestjs', label: 'NestJS', description: 'TypeScript, modular, enterprise', badge: 'popular' },
          { value: 'fastify', label: 'Fastify', description: 'Node.js, fast, schema validation', badge: 'popular' },
          { value: 'express', label: 'Express.js', description: 'Minimal, classic, lots of tutorials', badge: 'legacy' },
          { value: 'supabase-baas', label: 'Supabase (BaaS)', description: 'Skip custom backend, use Supabase', badge: 'recommended' },
      ...
      if (lang === 'python') {
        return [
          ...
          { value: 'django', label: 'Django', description: 'Batteries-included, built-in ORM', badge: 'popular' },
      ...
      if (lang === 'go') {
        return [
          ...
          { value: 'fiber', label: 'Fiber', description: 'Express-like, very fast' },
  ...
  // ══════════════════════════════════════════════════════════════
  {
    ...
    skipIf: (a) => {
      ...
      return isExtend(a) || backend === 'supabase-baas' || backend === 'firebase-baas' || backend === 'none';
    },
    required: true,
    default: (answers) => {
    ...
    options: (answers): Option[] => {
      ...
      const relational: Option[] = [
        ...
        { value: 'sqlite', label: 'SQLite', description: 'File-based, ideal for MVP / local dev', badge: mode === 'mvp' ? 'recommended' : undefined },
  ...
  // ══════════════════════════════════════════════════════════════
  {
    ...
    options: (answers): Option[] => {
      ...
      return [
        ...
        { value: 'clerk', label: 'Clerk', description: 'Hosted UI + user management, best DX', badge: 'recommended' },
        ...
        { value: 'firebase-auth', label: 'Firebase Auth', description: 'Google ecosystem' },
        { value: 'auth0', label: 'Auth0', description: 'Enterprise, managed', badge: 'popular' },
        { value: 'custom-jwt', label: 'Custom JWT', description: 'Build from scratch' },
